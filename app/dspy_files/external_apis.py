...
def fetch_all_cites_pages(taxon_name: str, api_key: str) -> dict:
    ...
    while True:
        ...
        if (
            ...
            or len(all_taxon_concepts) >= total_entries
        ):
            break
        current_page += 1
...
def fetch_iucn_red_list(
    taxon_rank: str, taxon_name: str, page: int, api_key: str, latest=True
) -> dict:
...
def get_name_pairs(common_name: str) -> list:
    ...
    name_pairs = [
        (species["sci_name"], species.get("common_name")) for species in species_list
    ]
...
def get_articles_by_date(
    api_key: str, keywords: str, from_date: str, to_date="", page=1
) -> dict:
...
def get_all_articles_by_date(
    api_key: str, keywords: str, from_date: str, to_date=""
) -> dict:
    ...
    while True:
        ...
        with open(
            "data/newsapi/news_articles_page_{}.json".format(current_page), "w"
        ) as f:
