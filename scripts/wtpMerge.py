...
def merge_and_deduplicate_csvs_by_type(base_path="data/WildlifeTradePortal"):
    ...
    for base_type, files in file_groups.items():
        ...
        print(f"  Removing exact duplicates from {base_type} (before adding source tracking)...")
    ...
    if len(results) >= 2:  # Need at least 2 datasets to join
        ...
        for base_type, result in results.items():
            ...
            data_cols = [col for col in df.columns if col not in ['source_folder', 'source_file']]
        ...
        for base_type, df in datasets.items():
            ...
            possible_cols = [col for col in df.columns if 'report' in col.lower() and 'id' in col.lower()]
            if possible_cols:
        ...
        if len(report_id_cols) >= 2:
            ...
            joined_df = joined_df.rename(columns={first_col: 'Report_ID'})  # Standardize column name
            ...
            for base_type, report_col in report_id_cols[1:]:
                ...
                print(f"    Result: {after_join} rows ({after_join - before_join} new rows)")
            ...
            print(f"  Final joined dataset: {len(joined_df)} rows, {len(joined_df.columns)} columns")
            ...
            print(f"  Cannot create joined dataset: Need Report ID columns in at least 2 datasets")
    ...
    with open(summary_file, 'w') as f:
        ...
        for base_type, result in results.items():
            ...
            for info in result['file_info']:
                f.write(f"  {info['folder']}/{info['file']}: {info['rows']} rows, {info['columns']} columns\n")
        ...
        if joined_df is not None:
            ...
            for base_type, info in join_info.items():
                f.write(f"  {base_type}: {info['original_rows']} rows, joined on '{info['join_column']}'\n")
    ...
    print(f"{'='*60}")
    for base_type, result in results.items():
        print(f"{base_type}: {result['final_count']} rows (removed {result['duplicates_removed']} duplicates)")
