...
def process_iucn_assessment(assessment: Dict) -> Dict:
    ...
    processed['scope_descriptions'] = "; ".join(scope_descriptions) if scope_descriptions else None
...
def convert_iucn_json_to_csv(input_file: str, output_file: str) -> None:
    ...
    try:
        ...
        if 'assessments' not in data:
            print("Error: This doesn't appear to be IUCN Red List data. No 'assessments' field found.")
            return
        
        # Process each assessment record
        assessments = data['assessments']
        processed_records = [process_iucn_assessment(assessment) for assessment in assessments]
        ...
        fieldnames = [
            'taxon_scientific_name', 'red_list_category_code', 'year_published', 'latest',
            'possibly_extinct', 'possibly_extinct_in_the_wild', 'sis_taxon_id', 
            'assessment_id', 'url', 'scope_descriptions', 'scope_codes'
        ]
...
def main():
    parser = argparse.ArgumentParser(description='Convert IUCN Red List JSON data to CSV')
