...
def process_taxon_record(taxon: Dict) -> Dict:
    ...
    flat_record["all_common_names"] = (
        "; ".join(all_common_names) if all_common_names else None
    )
...
def convert_speciesplus_json_to_csv(input_file: str, output_file: str) -> None:
    ...
    try:
        ...
        if "taxon_concepts" not in data:
            print(
                "Error: This doesn't appear to be Species+ API data. No 'taxon_concepts' field found."
            )
            return
